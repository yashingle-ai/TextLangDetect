...
# File paths
train_sentence_file = r"C:\\Users\\Lenovo\\OneDrive\\Desktop\\models and reports\\shuffle_test_train_data\\shuffled_train_sentences.txt"
train_label_file    = r"C:\\Users\\Lenovo\\OneDrive\\Desktop\\models and reports\\shuffle_test_train_data\\shuffed_train_labels.txt"
dev_sentence_file   = r"C:\\Users\\Lenovo\\OneDrive\\Desktop\\models and reports\\shuffle_test_train_data\\shuffled_dev_sentences.txt"
dev_label_file      = r"C:\\Users\\Lenovo\\OneDrive\\Desktop\\models and reports\\shuffle_test_train_data\\shuffed_dev_labels.txt"
test_sentence_file  = r"C:\\Users\\Lenovo\\OneDrive\\Desktop\\models and reports\\shuffle_test_train_data\\shuffled_test_sentences.txt"
test_label_file     = r"C:\\Users\\Lenovo\\OneDrive\\Desktop\\models and reports\\shuffle_test_train_data\\shuffed_test_labels.txt"
...
# Normalize labels
dev_labels = [normalize_mni(lab) for lab in dev_labels]
...
for combo in combos:
    ...
    try:
        ...
        for name, (model, vectorizer) in combo:
            ...
            # Use a clone of the model to avoid interference
            cloned_model = clone(model)
            pipeline = make_pipeline(clone(vectorizer), cloned_model)
            estimators.append((name, pipeline))
            transformed_devs.append(X_dev)
            transformed_tests.append(X_test)
        ...
        voting_clf.fit(dev_sentences, y_dev)

        pred_dev = [normalize_mni(p) for p in voting_clf.predict(dev_sentences)]
        pred_test = [normalize_mni(p) for p in voting_clf.predict(test_sentences)]
...
print("\nAll combinations evaluated and reports saved.")
