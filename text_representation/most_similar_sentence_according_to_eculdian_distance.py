...
def main():
    parser = argparse.ArgumentParser(description="Compute TF-IDF and sentence similarity using Euclidean distance.")
    parser.add_argument('--input_file', type=str, required=True, help="Path to the input text file.")
    parser.add_argument('--output_file', type=str, required=True, help="Path to save the similarity results.")
    ...
    with open(args.input_file, "r", encoding="utf-8") as f:
        ...
        with open(args.output_file, 'w', encoding='utf-8') as file:
            for first_sent in range(len(vector_contain_list)):
                sent_distance = float('inf')  # Initially set infinite
                ...
                for next_sent in range(len(vector_contain_list)):
                    ...
                    if distance_cur_sent < sent_distance:  # If it's less than previous sentences, assign the new values
                        matched_vector = next_sent  # Change the sentence
                ...
                if matched_vector == -1:
                    ...
                    file.write(f"Euclidean Distance: {sent_distance:.4f}\n")
