...
def proper_bullet_creation(text, pattern):
    ...
    if not bullet_patterns:
        ...
        print(len(updated_text) - len(text), len(bullet_patterns))
    return updated_text
...
def read_file_and_tokenize(input_file, lang_type=0):
    ...
    lines = remove_duplicates_from_lines(lines)
    pattern = '(\\d+\\.\\s?)'
...
def remove_duplicates_from_lines(lines):
    """Remove duplicates from lines."""
    dict_lines = {}
    for index, line in enumerate(lines):
        if line not in dict_lines:
            dict_lines[line] = index
    return sorted(dict_lines, key=dict_lines.get)
