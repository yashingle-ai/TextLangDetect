...
for first_sent in range(len(vector_contain_list)):
    ...
    for next_sent in range(len(vector_contain_list)):
        ...
        if  distance_cur_sent < sent_distance:                    #if its less then previous sentences then asign the new values 
    ...
    with open("C:\\Users\\yashi\\OneDrive\\Desktop\\ML project\\model_making\\text_representation\\euclidian_text_file.txt",'a',encoding='utf-8') as file :
        ...
        if matched_vector == -1:
            ...
            file.write(f"eculidean distance: {sent_distance:.4f}\n")
