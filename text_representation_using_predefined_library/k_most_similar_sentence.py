...
def main():
    ...
    with open(args.output_file,'w',encoding='utf-8')as f:
        for i,row in enumerate(sentence_similarity_score):
            ...
            # Exclude the sentence itself from similarity ranking
            sorted_indices = np.argsort(row)[::-1]  # Sort in descending order
            sorted_indices = sorted_indices[1:]    # Skip the first index (self-similarity) (slicing the list )
        
            K = min(args.k, len(sorted_indices))   # Handle case when k > number of sentences
            
            #writing sentence into your output file
             
            for q in range(K):
                most_similar_idx = sorted_indices[q]
                similarity_score = row[most_similar_idx]
                f.write(f"Most Similar to Sentence {most_similar_idx + 1}: {cleaned_sentences[most_similar_idx]}\n"f"Similarity Score: {similarity_score:.4f}\n\n")
