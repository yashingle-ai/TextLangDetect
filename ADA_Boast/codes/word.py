"""Identify Language of Texts in English and Indian Languages using AdaBoost"""
import argparse
...
def main():
    parser = argparse.ArgumentParser(description="AdaBoost Classifier with Decision Tree Base Estimator")
    ...
    for n_estimators in n_estimators_list:
        for learning_rate in learning_rates:
            model_prefix = f"adaboost-est{n_estimators}-lr{learning_rate}"
            print(f"Training model: {model_prefix}")

            base_estimator = DecisionTreeClassifier(max_depth=1)
            clf = AdaBoostClassifier(
                estimator=base_estimator,   # ← FIXED HERE
